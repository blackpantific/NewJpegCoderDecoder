...
namespace NewJpegCoderDecoder
{
	...
	public class NanoJPEG
	{
		...
		public static List<int[]> ToCoder { get; set; } = new List<int[]>();
		...
		public void njDecodeSOF()
		{
			...
			Program.Height = nj.height;
			nj.width = njDecode16(nj.pos + 3);
			Program.Width = nj.width;
			nj.ncomp = nj.posb[nj.pos + 5];
			Program.Comp = nj.ncomp;
			njSkip(6);
			switch (nj.ncomp)
			{
				case 1:
		...
		public void njDecodeBlock(nj_component_t c, int outv)
		{
			...
			int[] a = new int[64];
			for (int i = 0; i < nj.block.Length; i++)
			{
				a[i] = (short)nj.block[(int)njZZ[i]];
			}
		...
		public List<int[]> njDecode(byte[] jpeg)
		{
			try
			{
				njInit();
				...
				if (nj.error != nj_result_t.__NJ_FINISHED) return null;
				//
